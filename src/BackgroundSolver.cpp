...
BackgroundSolution solve_equations(Potential* pot, double N_star, double N_dagger)
{
    ...
    double phi_p = 0, N_temp_s = 0, N_temp_d = 0, N_end = 0;
    ...
    while(abs(N_temp_s - N_star) > 0.01 and abs(N_temp_d - N_dagger) > 0.01)
    {
        phi_p += 0.001;
        double t = t0;
        std::vector<double> x = {phi_p, dphi_p, 0, 0};
        ...
        x = {phi_p, dphi_p, 0, 0};
        desolver = dlsodar(4, 1, 1000000);
        desolver.integrate(t, 1e10, &x[0], equations, begin, static_cast<void*> (ptrs));
        double NN = x[2];
