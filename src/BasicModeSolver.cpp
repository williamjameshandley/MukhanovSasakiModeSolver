...
void BasicModeSolver::Construct_PPS_Scalar(double k_i, double k_f, double error = 1e-3)
{
    ...
    while(k_pair.size() != 0)
    {
        for(size_t n = 0; n < k_pair.size(); n++)
        {
            ...
            auto temp_approx1 = (PPS_Scalar(k_m1));
            auto temp_approx2 = (PPS_Scalar(k_m2));
            auto temp_approx3 = (PPS_Scalar(k_m3));
            auto temp_true1 = (Find_PPS_Scalar(k_m1));
            auto temp_true2 = (Find_PPS_Scalar(k_m2));
            auto temp_true3 = (Find_PPS_Scalar(k_m3));
    ...
    std::sort(k_plot_Scalar.begin(), k_plot_Scalar.end());
}

void BasicModeSolver::Construct_PPS_Tensor(double k_i, double k_f, double error = 1e-3)
{
    std::vector<std::pair<double, double>> k_pair;
    ...
    while(k_pair.size() != 0)
    {
        for(size_t n = 0; n < k_pair.size(); n++)
        {
            ...
            auto temp_approx1 = (PPS_Tensor(k_m1));
            auto temp_approx2 = (PPS_Tensor(k_m2));
            auto temp_approx3 = (PPS_Tensor(k_m3));
            auto temp_true1 = (Find_PPS_Tensor(k_m1));
            auto temp_true2 = (Find_PPS_Tensor(k_m2));
            auto temp_true3 = (Find_PPS_Tensor(k_m3));
    ...
    std::sort(k_plot_Tensor.begin(), k_plot_Tensor.end());
}
